 ...
 * */
function addComment() {
    var name = escapeHtml(document.getElementById("commenterName").value); //Get values in <input> for commenter name and the comment then encode
    var text = escapeHtml(document.getElementById("commenterText").value);
...
window.onload = function loadEntries() {
    ...
    if (currentURL.searchParams.get("title")) {
        var query = currentURL.searchParams.get("title"); //If the current window has the parameter "title" with non null/ empty query  

        //When using .text() JQuery uses CreateTextNode internally, which escapes all special characters
        //https://api.jquery.com/text/#text2

        $("#queryString").text("Searching for: " + query); //Set the query the user has input 
        document.getElementById("queryString").style.visibility = "visible"; //Show the query
        ...
        success: function (data, textStatus, jQxhr) {
            ...
            data.forEach(function (media) { //For each media entry 
                if (query == "" || media.title.includes(query)) { //If there is a query, match the title 
                    ...
                    //Persistent XML no longer exists here as Server side HTML encoding ensures it is not executed as code
                    //However, the <img> src attribute has to be encoded to ensure it cannot be escaped. 
                    $(".media .table").find("tbody").append("<tr><td><img class=\"poster\" src=" + escapeHtml(media.url) + "></td><td><h3>" + media.title + "<br></h3><p> " + media.description + "</p></td><td>" + media.type + "</td></tr>");
                }
            });
        },
        error: function (jqXhr, textStatus, errorThrown) {
...
/**
 * Purpose: Escape HTML and encode using entity references. Do not allow it to be run as code.
 * 
 * References: Based on answer at: https://stackoverflow.com/a/12034334
 * */
